...
import newproject.regression.det2;
...
public class MatrixMathematics {
	...
	public static Matrix transpose(Matrix matrix) {
            System.out.println("came to matrixmath transpose=");
		...
		for (int i=0;i<matrix.getNrows();i++) {
			for (int j=0;j<matrix.getNcols();j++) {
				transposedMatrix.setValueAt(j, i, matrix.getValueAt(i, j));
                 ...
                 double oo[][]=new double[matrix.getNrows()][matrix.getNcols()];
                            oo=matrix.getValues();
            for(int i=0;i<matrix.getNrows();i++)
        {
            for(int j=0;j<matrix.getNcols();j++)
        {
          ...
          System.out.println();   
            ...
            oo=transposedMatrix.getValues();
            for(int i=0;i<transposedMatrix.getNrows();i++)
        {
            for(int j=0;j<transposedMatrix.getNcols();j++)
        {
            System.out.print(oo[i][j]+" ");
        }
          System.out.println();   
	...
	public static Matrix inverse(Matrix matrix) throws NoSquareException
        {
            ...
            if(matrix.getNcols()==1 && matrix.getNrows()==1)
            {
                double a[][]=new double[1][1];
                a[0][0]=1/(matrix.getValueAt(0, 0));
                Matrix d=new Matrix(a);
                return d;
            }          
            else
            {
            System.out.println("came to inverse");
            double oo[][]=new double[matrix.getNrows()][matrix.getNcols()];
            oo=matrix.getValues();
            for(int i=0;i<matrix.getNrows();i++)
        {
            for(int j=0;j<matrix.getNcols();j++)
        {
          ...
          System.out.println();   
            ...
            double t=1.0/determinant(matrix);
            System.out.println("&&&&&&gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg&&&&&&&&&&&&&&&&&&&&t="+t);
	...
	public static double determinant(Matrix matrix) throws NoSquareException
        {
            c++;
            System.out.println("*********came to determinanssssst*************");
            det2 x=new det2();
            double o[][]=new double[matrix.getNrows()][matrix.getNcols()];
            o=matrix.getValues();
            int nx=matrix.getNrows();
            int nnx=matrix.getNcols();
             System.out.println(nx+" original matrix");
             System.out.println(nnx+" cols original matrix");
            for(int i=0;i<matrix.getNrows();i++)
            {
                for(int j=0;j<matrix.getNcols();j++)
                {
	...
	/*
		...
		//}
                //System.out.println("came out of determinant");
                    */
        System.out.println("detvalue="+f+"    c="+c);
            System.out.println("+++++++++++++++came out of determinantsss++++++++++++++");
	...
	public static Matrix createSubMatrix(Matrix matrix, int excluding_row, int excluding_col) 
        {
            System.out.println("came to subbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbmatrix");
            if(matrix.getNrows()==1 ||matrix.getNcols()==1)
               return matrix;
            else
            {
	...
	public static Matrix cofactor(Matrix matrix) throws NoSquareException 
		...
		for (int i=0;i<matrix.getNrows();i++) {
			for (int j=0; j<matrix.getNcols();j++) {
                ...
                double o[][]=new double[mat.getNrows()][mat.getNcols()];
            o=mat.getValues();
                System.out.println("cofactor done%%%%%%%%!@@##%#$$^$^$^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%%");
